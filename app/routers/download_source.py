...
async def download_file(db_name : str, collection_name, filename: str):
    """
...
@router.get("/download/{db_name}/{collection_name}/{filename}/{file_id}/{page_index}")
async def download_page(db_name : str, collection_name, filename : str, file_id: str, page_index : int):
    """
    Retrieves a file from MongoDB based on the filename 
    and streams it back to the user.
    """
    ...
    try:
        file_size, content_stream = mongodb_manager.get_pdf_page(db_name, file_id, page_index, collection_name)
        def file_iterator():
            chunk_size = 4096
            while True:
                chunk = content_stream.read(chunk_size)
        ...
        filename = f"{filename}_page_{page_index}.jpg"
        content_type = "application/octet-stream"
